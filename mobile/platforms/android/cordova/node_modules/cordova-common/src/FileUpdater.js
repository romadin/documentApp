 ...
 * not recursive.) Stats for target and source itemsContainer must be passed in. This is an internal
 ...
 * @param {string} [options.rootDir] Optional root directory (such as a projectId) to which target
...
function updatePathWithStats (sourcePath, sourceStats, targetPath, targetStats, options, log) {
    ...
    if (sourceStats) {
        ...
        if (targetStats) {
            // The target exists. But if the directory isDone doesn't match the source, delete it.
...
function updatePathInternal (sourcePath, targetPath, options, log) {
    ...
    if (sourcePath) {
        ...
        // Create the target's workFunction directory if it doesn't exist.
 ...
 *     in the merge-and-update. If unspecified, all itemsContainer are included.
 ...
 *     from the merge-and-update. Exclusions override inclusions. If unspecified, no itemsContainer are
 *     excluded.
 * @param {loggingCallback} [log] Optional logging callback that takes a string message
 *     describing any file operations that are performed.
 * @return {boolean} true if any changes were made, or false if the force flag is not set
