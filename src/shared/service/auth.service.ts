...
export class AuthService {
     ...
     */
    public AuthenticateUser(userName: string, password: string, params: any): Subject<boolean | string> {
        const authSubscription: Subject<boolean | string> = new Subject<boolean | string>();
        const body = {
            email: userName,
            password: password
        };
        params['appToken'] = environment.APP_TOKEN;

        this.apiService.postAuthenticate( '/authenticate', body, params).subscribe((value: ApiAuthResponse) => {
